...
**#//Tal vez deberíamos reemplazar d_work = 1 if hours > 0?. Lo mismo si wage > 0
...
* d_work: We add info from dum_work (pregunta ELPI) if d_work is missing.
forval t = 1/8 {
	replace d_work_t`t' = dum_work_t`t'		if d_work_t`t' == .
	replace d_work_t`t' = dum_work`t'_2010 if d_work_t`t' == .
	replace d_work_t`t' = dum_work`t'_2012 if d_work_t`t' == .
	replace d_work_t`t' = dum_work`t'_2017 if d_work_t`t' == .
}
