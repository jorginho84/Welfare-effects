...
*By income
forvalues x = 1/2{
	reghdfe public_34 min_center_NM $controls if cat_educ == `x', absorb(cohort#comuna_cod) vce(cluster comuna_cod)
	local beta_takeup_`x' =  string(round(-_b[min_center_NM]*100,.1),"%9.1f")
...
(scatter effects x, msymbol(circle) mcolor(black*.7) mfcolor(black*.7)) ///
	...
	xlabel(1 "Less than HS" 3 ">= HS", noticks) xscale(range(0.5 4))  ///
