...
**# Fig 1, panel b -- VERSION 2: BARRAS
use "$db/data_estimate", clear
...
format dist_min %9.0f

twoway (bar dist_min year, sort fcolor(none) lcolor(black)), scheme(lean2) graphregion(fcolor(none) ifcolor(none)) ytitle("Distance to the nearest center (mt)", size(large)) xtitle("Year", size(large))  ///
xlabel(2008(2)2014, /*noticks*/ labsize(large))  ylabel(500(250)1500, nogrid labsize(large)) /// 
graphregion(fcolor(white) ifcolor(white) lcolor(white) ilcolor(white)) ///
plotregion(fcolor(white) lcolor(white)  ifcolor(white) ilcolor(white)) 
graph export "$results/meandistance_year_24.pdf", as(pdf) replace 
